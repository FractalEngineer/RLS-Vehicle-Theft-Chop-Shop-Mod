...
-- Robust prototype for BeamNG.drive + RLS Career Overhaul.
-- Version intentionally remains 0.1.0 until release.
...
local cfg = {
  ...
  plateFlagDelaySeconds = 120,
  plateScanRadius = 55.0,
  plateScanInterval = 0.40,
  platePursuitLevel = 2,
  ...
  dealerHotwireReputationPenalty = 5000,
...
local function plateToken(text)
  text = string.lower(tostring(text or ""))
  return text:find("licenseplate", 1, true) ~= nil or text:find("license_plate", 1, true) ~= nil
end

local function capturePlateSlotsFromParts(parts)
  local result = {}
  if type(parts) ~= "table" then return result end

  for slotName, partName in pairs(parts) do
    if plateToken(slotName) or plateToken(partName) then
      local slot = tostring(slotName)
      -- Ignore decorative plate-design selectors; we only want physical slots.
      if not slot:lower():find("design", 1, true) then
...
local function getLiveParts(vehId)
  if not vehId or not core_vehicle_manager or not core_vehicle_manager.getVehicleData then return nil end
  local ok, vehicleData = pcall(function() return core_vehicle_manager.getVehicleData(vehId) end)
  if ok and vehicleData and vehicleData.config and type(vehicleData.config.parts) == "table" then
    return vehicleData.config.parts
  end
  return nil
end

local function capturePlateSlots(vehId, vehicle)
  local liveParts = getLiveParts(vehId)
  local slots = capturePlateSlotsFromParts(liveParts)
  if #slots > 0 then return slots end

  local savedParts = vehicle and vehicle.config and vehicle.config.parts
  return capturePlateSlotsFromParts(savedParts)
end

local function refreshPlateRemovedState(inventoryId, vehId, data)
  if not data or data.plateRemoved then return end
  if type(data.plateSlots) ~= "table" or #data.plateSlots == 0 then return end

  local parts = getLiveParts(vehId)
  if not parts then
    local vehicle = getInventoryVehicle(inventoryId)
    parts = vehicle and vehicle.config and vehicle.config.parts
  end
  if type(parts) ~= "table" then return end

  for _, slotName in ipairs(data.plateSlots) do
    local partName = parts[slotName]
    if partName ~= nil and tostring(partName) ~= "" then
      return
    end
  end

  data.plateRemoved = true
  data.plateRemovedAt = nowPersistentTime()
  markVehicleDirty(inventoryId)
  requestCareerSave(0.2)
  message("License plate removed — stolen plate scans disabled.", 4)
end
...
local function plainVehicleName(vehicle)
  if not vehicle then return "Stolen Vehicle" end
  if type(vehicle.niceName) == "string" and vehicle.niceName ~= "" then return vehicle.niceName end
  if type(vehicle.model) == "string" and vehicle.model ~= "" then return vehicle.model end
  return "Stolen Vehicle"
end

-- Keep a normal, structurally valid RLS insurance record, but force its policy
-- to -1. This is much safer than deleting the record because the inventory UI
-- expects the insurance module to know about every inventory vehicle.
...
local function ensureStolenInsuranceState(inventoryId)
  if not inventoryId or not isStolenInventoryVehicle(inventoryId) then return false end
...
local function completeCarjack(vehId, wasPoliceVehicle)
  ...
  -- Capture the exact live traffic config/paint before traffic teardown.
  ...
  vehicle.rlsCarjack = {
    ...
    plateRemoved = false,
    plateSlots = capturePlateSlots(vehId, vehicle),
    plateFlagAt = stolenAt + cfg.plateFlagDelaySeconds,
  ...
  if wasPoliceVehicle then
    ...
    message(string.format("Vehicle stolen. Plate goes hot in %ds.", cfg.plateFlagDelaySeconds), 5)
  end
  return true
end
...
local function completeSaleVehicleHotwire(candidate)
  ...
  -- it and later perform hot-plate detection.
  ...
  elseif orgId and repPenalty then
    ...
    message(string.format("Vehicle hotwired. Plate goes hot in %ds.", cfg.plateFlagDelaySeconds), 5)
  end
  return true
end
...
local function processPlateScan(dtReal)
  plateScanTimer = plateScanTimer - dtReal
  if plateScanTimer > 0 then return end
  plateScanTimer = cfg.plateScanInterval
  ...
  refreshPlateRemovedState(inventoryId, playerId, data)
  if data.plateRemoved then return end
  if nowPersistentTime() < (data.plateFlagAt or math.huge) then return end
  ...
  if not ok or not policeId or not distance or distance > cfg.plateScanRadius then return end

  local level = data.maximumHeat and cfg.policeCarjackLevel or cfg.platePursuitLevel
  pcall(function() police.setPursuitMode(level, playerId, {policeId}) end)
  message(data.maximumHeat and "STOLEN POLICE VEHICLE IDENTIFIED — MAXIMUM HEAT!" or "STOLEN PLATE HIT — police have identified the vehicle!", 5)
end
...
-- Restore the vehicle-local Strip for Parts action. It intentionally ignores
-- the normal needsRepair restriction so a retrieved wreck can be disposed of.
